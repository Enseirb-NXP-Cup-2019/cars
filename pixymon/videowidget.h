...
void VideoWidget::handleImage(QImage image, uchar renderFlags, QString desc) {
  // This code bumps the main window size up (if possible) to conform to the
  // actual frame dimension and aspect ratio
  ...
  // memcpy(dataSending, image.bits(), image.byteCount());
  //   write(connectedSocket, dataSending, image.byteCount());
  printf("img height: %d  ///// size = %d\n", image.height(),
         image.byteCount());
...
int VideoWidget::activeWidth() { return m_width; }

int VideoWidget::activeHeight() { return m_height; }
...
void VideoWidget::paintEvent(QPaintEvent* event) {
  ...
  float war;
  float pmar;
  QPainter p(this); // we could render to a QImage instead of to a widget.  This
                    // might take longer, but
  ...
  if (war > pmar) { // width is greater than video
    m_width   = this->height() * pmar;
    m_xOffset = (this->width() - m_width) / 2;
    m_yOffset = 0;
  } else { // height is greater than video
  ...
  for (i = 1; i < m_renderedLayers.size(); i++) {
    if (m_layerEnable[i])
      p.drawPixmap(QRect(m_xOffset, m_yOffset, m_width, m_height),
                   QPixmap::fromImage(m_renderedLayers[i].m_img));
  }
...
int VideoWidget::heightForWidth(int w) const { return w / m_aspectRatio; }
...
void VideoWidget::mouseMoveEvent(QMouseEvent* event) {
  ...
  if (m_drag && m_inputMode == REGION) {
    ...
    if (m_x0 - m_xOffset > 0 && m_y0 - m_yOffset > 0 &&
        m_x0 < m_xOffset + m_width && m_y0 < m_yOffset + m_height) {
      m_selection = true;
...
void VideoWidget::setupDialog(QTabWidget* tabs) {
  ...
  if (m_dialogTabs)
    ...
    // layer flags are not permanent, for now they are only applicable when
    // config dialog is up when the dialog goes away, we restore defaults.
    m_layerEnable.clear();
